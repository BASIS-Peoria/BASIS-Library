  ...
  const searchResults = useMemo(() => {
    ...
    for (const course of courses) {
      ...
      for (const vid of course.videos) {
        if (courseMatches || vid.title.toLowerCase().includes(q)) addItem(vid, "video", course.name);
      }
      for (const web of course.websites) {
        if (courseMatches || web.title.toLowerCase().includes(q)) addItem(web, "website", course.name);
      }
    }
    return results;
  }, [searchQuery]);
